            ...
            if columnInterp is None:
                ...
                for k, g in groups:
                    ...
                    if np.any(np.diff(xold) < 0):
                        ...
                        tmp = np.nonzero(np.diff(xold) < 0.0)[0]
                        idx_stop = tmp[0]
                        print(
                            "Non-monotonic for group: {:s}  Using x-axis sub-range {:g} - {:g}".format(
                                str(k), xold[idx_start], xold[idx_stop]
                            )
                        )

                    xnew_start = (
                        np.ceil(np.min(xold[idx_start:idx_stop] / interpStep))
                        * interpStep
                    )
                    xnew_stop = (
                        np.floor(np.max(xold[idx_start:idx_stop] / interpStep))
                        * interpStep
                        + 1e-6
                    )
                    xnew = np.round(
                        np.arange(xnew_start, xnew_stop, interpStep), 2
                    )  # To resolve floating point precision errors
