...
def fixMultiAxisLabel(ax, xindex):
    ...
    ax.set_xticklabels(tickLabels, rotation=tickRotation, fontsize=tickFontSize)
    ...
    for k in reversed(range(naxis - 1)):
        ...
        ax2.set_xticklabels(
            tlab, minor=True, rotation=tickRotation, fontsize=tickFontSize
        )
...
def plotMulti2(
    ...
    legendFields2 = pt[
        [valueField]
    ].columns.names  # Double brackets to make sure we get a Dataframe not a Series
    ...
    tickRotation, tickFontSize, positionIncrement = calculateTextProp(
        fig, axesWidth, tickLabels
    )

    ax1.set_xticks(range(len(dfToPlot)))
    ax1.set_xticklabels(tickLabels, rotation=tickRotation, fontsize=tickFontSize)
